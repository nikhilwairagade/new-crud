...
router.get('/SearchByName/', auth, async (req, res) => {
	if (!req.query.name) return res.status(400).send('Invalid Request');
	const createremail = req.user.email;
...
router.get('/SearchByAge/', auth, async (req, res) => {
	...
	if (isNaN(req.query.ageAfter) || isNaN(req.query.ageBefor)) return res.status(400).send('Invalid Request');
	const createremail = req.user.email;
...
router.post('/', auth, async (req, res) => {
	...
	await student.save();
	res.status(200).json((_.pick(req.body, ['name', 'age', 'dob', 'semail', 'college', 'createremail'])));
});
